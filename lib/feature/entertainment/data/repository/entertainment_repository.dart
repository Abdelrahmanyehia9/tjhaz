...
class EntertainmentRepository{
  ...
  Future<Either<EntertainmentDetailsModel , String>>getEntertainmentDetailsByID({required String entertainmentID })async{
try{
  final result = await firestore.collection("entertainment").doc(entertainmentID).get() ;
  EntertainmentDetailsModel entertainment = EntertainmentDetailsModel.fromJson(result.data()!) ;
  ...
  Future<Either<List<EntertainmentDetailsModel> ,String >> getEntertainmentsByCategoryID({required String catID})async{
    try{
      List<EntertainmentDetailsModel> entertainments = [] ; 
      final response = await firestore.collection(FireStoreConstants.entertainment).where("category_id" , isEqualTo: catID).get() ;
